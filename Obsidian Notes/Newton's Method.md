Newton’s Method iteratively approximates the root (zero) of a function $f(x)$ by starting with an initial guess and applying the formula: $$x_{n+1} = x_{n} - \frac{f(x_{n})}{f'(x_{n})}$$
